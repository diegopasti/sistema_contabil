...
from entidade.models import Logradouro, endereco_serializer
...
def consultar_codigo_postal(cep):
    ...
    if resultado.count() != 0:
        ...
        resultado = json.load(urllib2.urlopen("http://127.0.0.1:8081/api/consultar_cep/"+cep+"/"))
...
def consultar_bairro(nome,codigo_bairro,municipio):
    from entidade.models import Bairro
...
def consultar_municipio(nome,codigo_municipio,estado):
    from entidade.models import Municipio    
...
def consultar_estado(sigla):
    from entidade.models import Estado
...
def consultar_pais(sigla):
    from entidade.models import Pais
...
def consultar_codigo_postal_default(cep):
    ...
    if resultado['resultado'][0] == '1':
        ...
        bairro = unicode(resultado['bairro'][0].upper().decode("latin-1"))
        
        cidade = unicode(resultado['cidade'][0].upper().decode("latin-1"))
